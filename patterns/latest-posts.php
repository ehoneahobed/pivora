	...
	<div class="wp-block-group alignwide pivora-posts-section__inner">
		...
		<div class="wp-block-group pivora-posts-section__header">
			...
			<div class="wp-block-buttons pivora-posts-section__action">
				<!-- wp:button {"className":"is-style-outline","url":"<?php echo esc_url( $blog_url ); ?>"} -->
				<div class="wp-block-button is-style-outline"><a class="wp-block-button__link wp-element-button" href="<?php echo esc_url( $blog_url ); ?>"><?php esc_html_e( 'View Articles', 'pivora' ); ?></a></div>
				<!-- /wp:button -->
			</div>
			<!-- /wp:buttons -->
		</div>
		...
		<?php if ( pivora_is_core_active() ) : ?>
			...
			pivora_block(
				...
				array(
					...
					'showDate'          => true,
					'excerptLength'     => 22,
					'readMoreText'      => __( 'Read article', 'pivora' ),
				)
			);
			?>
		<?php else : ?>
